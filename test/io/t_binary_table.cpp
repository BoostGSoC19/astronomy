...
namespace fits_test {
    ...
    public:
        basic_binary_table_extension<card_policy,data_conversions> binary_table1;
    ...
    private:
        ...
        void initialize_binary_table_hdu(basic_binary_table_extension<card_policy, data_conversions>& binary_table_hdu, const std::string& sample_name) {
            hdu_store<card_policy>* raw_binary_table_sample = get_raw_hdu(sample_name, "BINTABLE");
            if (raw_binary_table_sample != nullptr) {
                binary_table_hdu = basic_binary_table_extension<card_policy, data_conversions>(raw_binary_table_sample->hdu_header, raw_binary_table_sample->hdu_data_buffer);
            }
        }
    };
}
...
BOOST_FIXTURE_TEST_CASE(binary_table_set_data, fits_test::binary_table_fixture) {
    ...
    basic_binary_table_extension<card_policy, data_conversions> test_hdu(raw_bin_hdu->hdu_header,"");
...
BOOST_AUTO_TEST_CASE(binary_table_check_column_size) {
    auto col_size = basic_binary_table_extension<card_policy, data_conversions>::column_size("144000I");
...
BOOST_AUTO_TEST_CASE(binary_table_check_element_count) {
    auto element_count = basic_binary_table_extension<card_policy, data_conversions>::element_count("300I");
    BOOST_REQUIRE_EQUAL(element_count, 300);
    
}
BOOST_AUTO_TEST_CASE(binary_table_check_element_count_with_no_number) {
    auto element_count = basic_binary_table_extension<card_policy, data_conversions>::element_count("I");
    BOOST_REQUIRE_EQUAL(element_count, 1);
}
BOOST_AUTO_TEST_CASE(binary_table_get_type) {
    auto type = basic_binary_table_extension<card_policy, data_conversions>::get_type("242000I");
    BOOST_REQUIRE_EQUAL(type, 'I');
}

BOOST_AUTO_TEST_CASE(binary_table_type_size) {
    auto type_size = basic_binary_table_extension<card_policy, data_conversions>::type_size('A');
    BOOST_REQUIRE_EQUAL(type_size, 1);
 
}
BOOST_AUTO_TEST_CASE(binary_table_type_size_invalid_type) {

    BOOST_REQUIRE_THROW((basic_binary_table_extension<card_policy,data_conversions>::type_size('G')), boost::astronomy::invalid_table_colum_format);
}
